    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
      <div class="px-4 py-5 sm:px-6 flex justify-between">
        ...
        <div class="col-md-12 text-right mb-3">
          <button id="download" v-on:click="downloadPdf()" class="bg-green-500 border border-transparent rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 mr-4">Accept and Download</button>
          <button v-on:click="declinefunction()" class="bg-red-500 border border-transparent rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 mr-4">Decline</button>
        </div>
      </div>
...
import { getMortgageInterestOffer, getMortgageProcessState, InterestOffer, MortgageProcessState } from "../../modules/api/Paths/mortgageRequest";
...
import Breadcrumbs from "@/main/public/components/Breadcrumbs.vue";
...
export default class PageBindingOfferCustomer extends Vue {
  static tagName ="PageBindingOfferCustomer";
 ...
 dmortgageOfferID = this.mortgageId as number;
  ...
  async mounted(){
    ...
    // GET all the application form information:
    ...
    //GET all the interest offer information
    const InterestRes = await getMortgageInterestOffer(this.mortgageId);
    switch (InterestRes.status){
      case Status.OK:
        break;
      default: {
        console.error(InterestRes.status, InterestRes.statusText, InterestRes);
      }return;
    }
    this.interestOffer = InterestRes.content;

    //GET all the mortgage process status, state information
    const mortgageRes = await getMortgageProcessState(this.mortgageId);
    switch (mortgageRes.status){
      case Status.OK: break;
      default: {
        console.error(mortgageRes.status, mortgageRes.statusText, mortgageRes);
      }return;
    }
    this.mortgage = mortgageRes.content;

    this.totalInterest = (this.interestOffer.interest_rate * this.application.requestedMortgageAmount)/100;
  ...
  declinefunction() {
    const popup = confirm(" This action will delete your mortgage. Do you want to decline for this binding offer ?");
    if(!popup){return}
    location.hash = `#mymortgage`;
    changeStatus(new ProcessState(
        this.mortgageId,
        ProcessStateEnum.BindingOffer,
        ProcessStatusEnum.Failed
    ));
  }

  downloadPdf(){
    const popup = confirm("Do you agree with these information ?");
    if(!popup){return}
    ...
    const collateralValue = this.application?.price1 + this.application?.price2 + this.application?.price3 as number;
    const interest_rate = this.interestOffer?.interest_rate as number;
    const duration = this.interestOffer?.duration as string;

    const totalcost = valueloan + interest_rate * valueloan; //valueloan + interest


    // Writing PDF Template for Binding Offer:
    doc.setFont("arial").setFontSize(12);

    doc.text("Mortgage ID: " + mortgageOfferID, 30, 20);
    doc.text("Application Date: " + applicationDate, 128, 20).setLineHeightFactor(1.5);

    doc.text("Dear " + customerName + "," , 30, 50);
    doc.text("You are looking for a mortgage that suits your situation and wishes. That is why you have"
        ...
        + "\n\nWe offer you a mortgage of €" + valueloan + ". Detailed information about your mortgage can"
    ...
    doc.addPage().setFontSize(14).setTextColor(49, 62, 140).setLineHeightFactor(2);

    doc.text("DETAILS OF MORTGAGE OFFER", 30, 35);
    doc.line(30, 40, 175, 40).setFontSize(12).setTextColor(0, 0, 0);
    doc.text("Name applicant:				" + customerName
        + "\nAmount of the loan:			  " + valueloan
        + "\nValue of the house:			   " + collateralValue
        + "\nRepayment method:			  " + "fixed-rate mortgage"
        + "\ninterest rate:				      " + interest_rate + "%"
        + "\nfixed rate period:			      " + duration, 30, 60);

    doc.addPage().setFontSize(14).setTextColor(49, 62, 140).setLineHeightFactor(1.5);

    doc.text("ADDITIONAL ONE-OFF COSTS", 30, 35);
    doc.line(30, 40, 175, 40).setFontSize(12).setTextColor(0, 0, 0);

    doc.text("APPRASIALS COSTS: the collateral must be appraised by an appraiser acceptable to us."
        ...
        + "\ncompletely with you own resources.", 30, 50).setFontSize(14).setTextColor(49, 62, 140);
    ...
    doc.text("Early repayment of the financing is always possible. You can make additional"
        ...
        + "\nfee. For transfering your mortgage to the cureent interest rate you pay a fee of €350.", 30, 165).setFontSize(14).setTextColor(49, 62, 140);

    doc.text("CODE OF CONDUCT MORTGAGE FINANCES", 30, 215).setFontSize(12).setTextColor(0, 0, 0);
    doc.line(30, 220, 175, 220).setFontSize(12).setTextColor(0, 0, 0);

    doc.text("We signed the Code of Conduct for Mortgage Financing. You can request this code"
        + "\nof Conduct by us. A mortgage advisor of us can provied you with more information about"
        + "\nthe Code of Conduct and the possible consequences in your situation.", 30, 230);
    ...
    doc.text("Complaints about compliance with the code of conduct can be submitted in writtin"
        ...
        + "\nPostbus 93257"
        + "\n2509 AG Den Haag"
        + "\nFor more information: www.kifid.nl.", 30, 50).setFontSize(14).setTextColor(49, 62, 140);

    doc.text("GENERAL INFORMATION", 30, 160);
    doc.line(30, 165, 175, 165).setFontSize(12).setTextColor(0, 0, 0);

    doc.text("This offer is made on the condition that all information requested by us regarding"
        ...
        + "\nin Fraud Testing System. This is mostly for the benefit of third parties.", 30, 175);
    ...
    doc.text("SIGNING THE OFFER. By accepting this offer you declare that you agree with the entire"
        ...
        + "\n\n\nSignature applicant:"
        + "\n\n\n\n__________________________", 30, 50);


    doc.save("BindingOffer" + mortgageOfferID);

    location.href = "http://localhost:8080/#tracktrace?id=" + this.mortgageId;
  }
